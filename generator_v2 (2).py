...
def generate_image(prompt: str, input_image_bytes: bytes = None) -> bytes:
    from openai import OpenAI
    import base64
    client = OpenAI(api_key=OPENAI_KEY)

    if input_image_bytes:
        # Edit existing image with gpt-image-1
        import io as _io
        img_file = _io.BytesIO(input_image_bytes)
    ...
    if response.data[0].b64_json:
        ...
        return requests.get(response.data[0].url, timeout=30).content
...
async def generate_by_game(update, context, game: str, geo: str, offer: str, count: int, chat_id: int, input_photo: bytes = None):
    ...
    for i, variant in enumerate(variants_to_use):
        try:
            ...
            image_bytes = await loop.run_in_executor(None, generate_image, prompt, input_photo)
